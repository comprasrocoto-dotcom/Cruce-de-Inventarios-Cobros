/**
 * Trazabilidad Histórica. Agrupa los movimientos por periodos y compara la
 * evolución de la confiabilidad e impacto económico entre periodos, por sede,
 * por centro de costo y de forma global. Corresponde a la pestaña Trazabilidad.
 */
...
export const HistoricalTraceability: React.FC<HistoricalTraceabilityProps> = ({
  data,
  sedes,
  ccs,
  subfamilias,
  ...
  const downloadExcel = () => {
    ...
    // Sede sheet
    const sedeData: any[] = [];
    (Object.entries(historicalData.bySede) as [string, HistoricalPeriodStats[]][]).forEach(
      ([sede, sedeStats]) => {
        sedeStats.forEach((s) => {
          sedeData.push({
            Sede: sede,
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-[#0F1C2E] text-white p-8 rounded-[12px] shadow-lg relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-[#C9D4E3] font-medium">
            Comparativo histórico de confiabilidad e impacto económico
          </p>
        </div>
      ...
      {fileName && (
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h4 className="text-emerald-900 font-bold uppercase text-xs tracking-wider">
                ARCHIVO HISTÓRICO CARGADO CORRECTAMENTE
              </h4>
              ...
              <p className="text-emerald-600 text-[10px] font-bold uppercase mt-1">
                {data.length} registros históricos procesados con éxito
              </p>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          <div className="bg-white p-5 rounded-[12px] border border-brand-border shadow-sm">
            ...
            <div className="flex items-end gap-2">
              <p className="text-2xl font-bold text-text-main">
                {kpis.currentReliability.toFixed(1)}%
              </p>
              <span
                className={`text-[10px] font-bold mb-1 flex items-center ${kpis.accumulatedVariation >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}
              >
                {kpis.accumulatedVariation >= 0 ? (
          ...
          <div className="bg-white p-5 rounded-[12px] border border-brand-border shadow-sm">
            ...
            <p className="text-[10px] font-bold text-text-secondary uppercase">
              {kpis.bestPeriod.confiabilidad.toFixed(1)}% confiabilidad
            </p>
          </div>
          <div className="bg-white p-5 rounded-[12px] border border-brand-border shadow-sm">
            <p className="text-[10px] font-bold text-text-secondary uppercase tracking-widest mb-1">
            ...
            <p className="text-[10px] font-bold text-text-secondary uppercase">
              {kpis.worstPeriod.confiabilidad.toFixed(1)}% confiabilidad
            </p>
          </div>
          <div className="bg-white p-5 rounded-[12px] border border-brand-border shadow-sm">
            <p className="text-[10px] font-bold text-text-secondary uppercase tracking-widest mb-1">
            ...
            <p className="text-xl font-bold text-text-main">
              {formatCurrency(kpis.accumulatedImpact)}
            </p>
            <p className="text-[10px] font-bold text-text-secondary uppercase">
              Pérdida total en rango
            </p>
          </div>
          <div className="bg-white p-5 rounded-[12px] border border-brand-border shadow-sm">
            <p className="text-[10px] font-bold text-text-secondary uppercase tracking-widest mb-1">
            ...
            <p
              className={`text-xl font-bold ${kpis.accumulatedVariation >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
            >
              {kpis.accumulatedVariation >= 0 ? '+' : ''}
              {kpis.accumulatedVariation.toFixed(1)}%
            </p>
            <p className="text-[10px] font-bold text-text-secondary uppercase">
              Desde el inicio del rango
            </p>
          </div>
          <div className="bg-white p-5 rounded-[12px] border border-brand-border shadow-sm">
            <p className="text-[10px] font-bold text-text-secondary uppercase tracking-widest mb-1">
            ...
            <div className="flex items-center gap-2">
              ...
              <p
                className={`text-xl font-bold ${kpis.trend === 'Mejorando' ? 'text-emerald-600' : kpis.trend === 'Empeoró' ? 'text-rose-600' : 'text-text-secondary'}`}
              >
                {kpis.trend}
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-text-secondary leading-relaxed">
                  En el rango analizado, la confiabilidad pasó de{' '}
                  <span className="font-bold text-text-main">
                    {firstStats.confiabilidad.toFixed(1)}%
                  </span>{' '}
                  a{' '}
                  <span className="font-bold text-text-main">
                    {currentStats.confiabilidad.toFixed(1)}%
                  </span>
                  , mejorando{' '}
                  <span
                    className={`font-bold ${kpis.accumulatedVariation >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                  >
                    {Math.abs(kpis.accumulatedVariation).toFixed(1)} puntos porcentuales
                  </span>
                  .
                </p>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-1.5 h-1.5 rounded-full bg-[#38BDF8] mt-1.5 flex-shrink-0"></div>
                <p className="text-text-secondary leading-relaxed">
                  El impacto económico disminuyó de{' '}
                  <span className="font-bold text-text-main">
                    {formatCurrency(firstStats.impactoEconomico)}
                  </span>{' '}
                  a{' '}
                  <span className="font-bold text-rose-600">
                    {formatCurrency(currentStats.impactoEconomico)}
                  </span>
                  .
                </p>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-1.5 h-1.5 rounded-full bg-[#38BDF8] mt-1.5 flex-shrink-0"></div>
                <p className="text-text-secondary leading-relaxed">
                  El período con peor desempeño fue{' '}
                  <span className="font-bold text-rose-600 uppercase">
                    {kpis.worstPeriod.period}
                  </span>{' '}
                  con un{' '}
                  <span className="font-bold">{kpis.worstPeriod.confiabilidad.toFixed(1)}%</span>.
                </p>
              </div>
            </div>
            <div className="space-y-4">
              {Object.entries(historicalData.bySede).length > 0 && (
                <div className="flex items-start gap-3">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#38BDF8] mt-1.5 flex-shrink-0"></div>
                  <p className="text-text-secondary leading-relaxed">
                    La sede con mejor evolución fue{' '}
                    <span className="font-bold text-[#38BDF8] uppercase">
                      {
                        Object.entries(historicalData.bySede)
                          .map(([name, s]) => ({
                            name,
                            var: (s[s.length - 1]?.confiabilidad || 0) - (s[0]?.confiabilidad || 0),
                          }))
              ...
              {Object.entries(historicalData.byCC).length > 0 && (
                <div className="flex items-start gap-3">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#38BDF8] mt-1.5 flex-shrink-0"></div>
                  <p className="text-text-secondary leading-relaxed">
                    El centro de costos con mayor mejora fue{' '}
                    <span className="font-bold text-[#38BDF8] uppercase">
                      {
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={stats}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#1a2d45" />
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={[]}>
                ...
                />
                {(Object.entries(historicalData.bySede) as [string, HistoricalPeriodStats[]][]).map(
                  ([sede, s], idx) => (
                    <Line
                      key={sede}
                      data={s}
            ...
            <tbody className="divide-y divide-brand-border">
              {stats.map((s) => (
                <tr key={s.period} className="hover:bg-[#132238] transition-colors">
                  ...
                  <td className="px-4 py-4 text-center text-text-secondary text-sm font-bold">
                    {s.evaluados}
                  </td>
                  <td className="px-4 py-4 text-center text-emerald-600 text-sm font-bold">
                    {s.sinDiferencia}
                  </td>
                  <td className="px-4 py-4 text-center text-rose-600 text-sm font-bold">
                    {s.conDiferencia}
                  </td>
                  <td className="px-4 py-4 text-center">
                    <span
                      className={`font-bold text-sm ${s.confiabilidad >= 85 ? 'text-emerald-600' : s.confiabilidad >= 70 ? 'text-amber-500' : 'text-rose-600'}`}
                    >
                      {s.confiabilidad.toFixed(1)}%
                    </span>
                  </td>
                  <td className="px-4 py-4 text-right font-bold text-text-main text-sm">
                    {formatCurrency(s.impactoEconomico)}
                  </td>
                  <td className="px-4 py-4 text-center text-rose-500 text-xs font-bold">
                    {s.faltantes}
                  </td>
                  <td className="px-4 py-4 text-center text-emerald-500 text-xs font-bold">
                    {s.sobrantes}
                  </td>
                  <td className="px-4 py-4 text-center">
                    {s.variacionVsAnterior !== undefined ? (
                      <span
                        className={`text-xs font-bold flex items-center justify-center gap-1 ${s.variacionVsAnterior >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                      >
                        {s.variacionVsAnterior >= 0 ? (
                          <ArrowUpRight className="w-3 h-3" />
                        ) : (
                          <ArrowDownRight className="w-3 h-3" />
                        )}
      ...
      <div className="space-y-8">
            ...
            {(Object.entries(historicalData.bySede) as [string, HistoricalPeriodStats[]][]).map(
              ([sede, s]) => {
                ...
                return (
                  <div
                    key={sede}
                    className="bg-white p-5 rounded-[12px] border border-brand-border shadow-sm hover:shadow-md transition-shadow"
                  >
                    <h4 className="font-bold text-text-main text-sm uppercase mb-4 border-b pb-2">
                      {sede}
                    </h4>
                    <div className="space-y-2 mb-4">
                      {s.slice(-3).map((period) => (
                        ...
                        >
                          <span className="text-text-secondary">{period.period}</span>
                          <span
                            className={`font-bold ${period.confiabilidad >= 85 ? 'text-emerald-600' : period.confiabilidad >= 70 ? 'text-amber-500' : 'text-rose-600'}`}
                          >
                    ...
                    <div className="pt-3 border-t border-brand-border space-y-1">
                      <div className="flex justify-between text-[10px] font-bold uppercase">
                        <span className="text-text-secondary">Variación:</span>
                        <span className={variation >= 0 ? 'text-emerald-600' : 'text-rose-600'}>
                          {variation >= 0 ? '+' : ''}
                          {variation.toFixed(1)} pts
                        </span>
                      </div>
                      <div className="flex justify-between text-[10px] font-bold uppercase">
                      ...
                      <div className="flex justify-between text-[10px] font-bold uppercase">
                        <span className="text-text-secondary">Tendencia:</span>
                        <span
                          className={
                            trend === 'Mejorando'
                              ? 'text-emerald-600'
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {(Object.entries(historicalData.byCC) as [string, HistoricalPeriodStats[]][]).map(
              ([cc, s]) => {
                const first = s[0]?.confiabilidad || 0;
                const last = s[s.length - 1]?.confiabilidad || 0;
                const variation = last - first;
